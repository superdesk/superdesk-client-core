    ...
    return {
        ...
        link: function (scope, elem) {
            ...
            function queryItems(queryString?, params?) {
                ...
                api.query(getProvider(criteria), criteria)
                    ...
                    .finally(() => {
                        ...
                        if (
                            $rootScope.config.features.customMonitoringWidget &&
                            !scope.selected &&
                            scope.itemIds &&
            ...
            scope.$on('item:move', (_e, data) => {
                if (
                    (data.to_desk && data.from_desk !== data.to_desk) ||
            ...
            elem.bind('scroll', (event) => {
                if (scope.fetching) {
                    // ignore scrolling while fetching
                    event.preventDefault();
                    return false;
                }

                if (
                    container.scrollTop + container.offsetHeight >= container.scrollHeight - 3 &&
            ...
            scope.move = function (diff, event) {
                if (!_.isNil(scope.selected) && $rootScope.config.features.customMonitoringWidget && scope.itemIds) {
                    ...
                    if (index === -1) {
                        // selected not in current items, select first
                        container.scrollTop = 0;
                    ...
                    if (index !== nextIndex) {
                        ...
                        if (
                            (nextIndex + 2) * itemHeight > container.offsetHeight + container.scrollTop &&
                            nextIndex > index
                        ) {
