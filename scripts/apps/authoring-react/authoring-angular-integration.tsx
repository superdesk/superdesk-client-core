...
import {IArticle, ITopBarWidget, IExposedFromAuthoring, IAuthoringOptions, IAuthoringActionType} from 'superdesk-api';
...
function getInlineToolbarActions(
    ...
    const getManageHighlights = (): ITopBarWidget<IArticle> => ({
        ...
        component: () => (
            <WithPopover
                component={({closePopup}) => <HighlightsCardContent close={closePopup} article={item} />}
                placement="right-end"
            >
                {(togglePopup) => (
                    <IconButton
                        onClick={(event) => togglePopup(event.target as HTMLElement)}
                        icon={item.highlights.length > 1 ? 'multi-star' : 'star'}
    ...
    const getReadOnlyAndArchivedFrom = (): Array<ITopBarWidget<IArticle>> => {
        ...
        if (
            ...
            actions.push({
                group: 'start',
                availableOffline: false,
                component: () => <Label text={gettext('Read-only')} style="filled" type="warning" />,
                priority: 0.3,
            });
        }
            ...
            <Button
                ...
                onClick={() => {
                    showModal(({closeModal}) => (
                        ...
                        >
                            {gettext('Are you sure you want to unpublish item "{{label}}"?', {
            ...
            const manageDesksButton: ITopBarWidget<IArticle> = {
                ...
                component: () => (
                    <>
                        ...
                        >
                            <MarkedDesks article={item} />
                        </Popover>
                        <NavButton onClick={() => null} id="marked-for-desks" icon="bell" iconSize="small" />
                    </>
                ),
                availableOffline: true,
            };
                    ...
                    <NavButton
                        ...
                        onClick={() => {
                            if (panelState.active) {
                                ...
                                const availableTabs: Array<IArticleActionInteractive> = ['send_to'];
...
export function getAuthoringPrimaryToolbarWidgets(
    ...
    return Object.values(extensions)
        ...
        .map((item): ITopBarWidget<IArticle> => {
            ...
            return {
                ...item,
                component: (props: {entity: IArticle}) => <Component entity={props.entity} />,
            };
        })
        ...
        return (
            <div className="sd-authoring-react">
                <AuthoringIntegrationWrapper
                    ...
                    onClose={onClose}
                    getInlineToolbarActions={(exposed) =>
