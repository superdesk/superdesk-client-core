...
export function editMetadata(item: IArticle, mode: 'view' | 'image-edit' | 'crop'): Promise<IArticle> {
    const cropOptions = {
        isNew: false,
        editable: true,
        isAssociated: true,
     ...
     */
    return ng.get('renditions').crop(cloneDeep(item), cropOptions, {immutable: true});
}
