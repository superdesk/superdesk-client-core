                ...
                <Card>
                    <Spacer v gap="8" noWrap>
                        <Spacer h gap="8" noWrap>
                            ...
                            {showVersion && <div>{gettext('Version: {{n}}', {n: item.version})}</div>}
                        </Spacer>
                ...
                {historyItems.map((item, i) => {
                    if (item.operation === 'create') {
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Marked for highlight {{x}} by {{user}}', {
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Unmarked from highlight({{x}}) by {{user}}', {
                                        x: this.getHighlightsLabel(
                                            item.update.highlight_id,
                                            item.update.highlight_name,
                                        ),
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Marked for desk {{x}} by {{user}}', {
                                        x: this.getHighlightsLabel(
                                            item.update.highlight_id,
                                            item.update.highlight_name,
                                        ),
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Unmarked from desk({{x}}) by {{user}}', {
                                        x: this.getHighlightsLabel(
                                            item.update.highlight_id,
                                            item.update.highlight_name,
                                        ),
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Created from highlight({{x}}) by {{user}}', {
                                        x: this.getHighlightsLabel(
                                            item.update.highlight_id,
                                            item.update.highlight_name,
                                        ),
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>{gettext('Linked by {{user}}', {user: item.displayName})}</div>
                        ...
                        return (
                            <BaseHistoryItem items={historyItems} current={i} key={i}>
                                <div>
                                    {gettext('Correction cancelled by {{user}}', {
                                        user: item.displayName,
                                    })}
                                </div>

                                {item.update != null && <div>{gettext('Correction link is removed')}</div>}
                            </BaseHistoryItem>
                        );
