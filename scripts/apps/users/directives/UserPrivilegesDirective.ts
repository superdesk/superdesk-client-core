...
export function UserPrivilegesDirective(api, notify, userList, $q) {
    return {
        ...
        link: function (scope) {
            getUser().then(getPrivileges).then(getUserRole);

            function getUser() {
                return userList.getUser(scope.user._id, true).then(
                    (u) => {
                        scope.user = u;
             ...
             */
            scope.save = function () {
                api.save('users', scope.user, _.pick(scope.user, 'privileges')).then(
                    () => {
                        scope.origPrivileges = angular.copy(scope.user.privileges);
            ...
            scope.cancel = function () {
                scope.user.privileges = angular.copy(scope.origPrivileges);
