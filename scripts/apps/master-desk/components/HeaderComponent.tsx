...
import {CheckButtonGroup, RadioButtonGroup, Switch, ButtonGroup, Button} from 'superdesk-ui-framework/react';
...
export class HeaderComponent extends React.Component<IProps, IState> {
    ...
    componentDidMount() {
        Promise.all([this.services.preferences.get(USER_PREFERENCE_SETTINGS), this.services.desks.initialize()]).then(
            (res) => {
                const [preferences] = res;

                this.setState({availableDesks: this.services.desks.desks._items});

                this.services.desks.desks._items.map((item) =>
                    this.deskDropdownItems.push({
                        label: item.name,
                        onSelect: () => this.props.onDeskChange(item),
                    }),
                );
                ...
                if (preferences) {
                    this.setState({
                        ...
                        showAllDesks: preferences.showAllDesks === undefined ? true : preferences.showAllDesks,
                    });
                }
            },
        );
    ...
    render() {
        ...
        const tabs = Object.values(IMasterDeskTab)
            .filter((tab) => (this.props.isPlaningActive ? tab : !planningComponents.includes(tab)))
            .map((tab) => ({value: tab, label: getLabelForMasterDeskTab(tab)}));
            ...
            <div className="sd-main-content-grid__header">
                <div className="subnav">
                    {this.props.selectedDesk != null ? (
                        <React.Fragment>
                            <div className="flat-searchbar">
                                <button className="navbtn navbtn--left" onClick={() => this.props.onDeskChange(null)}>
                    ...
                    {this.props.isFilterAllowed && this.props.selectedDesk == null ? (
                        <button
                            className={
                                'sd-navbtn sd-navbtn--left sd-navbtn--darker' +
                    ...
                    {this.props.selectedDesk != null && this.isDeskMember(this.props.selectedDesk) ? (
                    ...
                    <div className={'dropdown' + (this.state.openDeskDropdown ? ' open' : '')}>
                        <button
                            className="sd-navbtn"
                            onClick={() => this.setState({openDeskDropdown: !this.state.openDeskDropdown})}
                        >
