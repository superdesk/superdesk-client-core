...
describe('sdIngestSourcesContent directive', () => {
    ...
    beforeEach(inject(($compile, $rootScope, $templateCache, ingestSources, $q) => {
        spyOn(ingestSources, 'fetchAllFeedingServicesAllowed').and.returnValue(
            Promise.resolve([
                {
                    feeding_service: 'rss',
                    label: 'RSS',
                    fields: [
                        ...
                        },
                        {
                            id: 'username',
                            type: 'text',
                            show_expression: 'provider.config.auth_required',
                        },
                    ],
                },
            ]),
        ...
        var templateUrl = ['scripts', 'apps', 'ingest', 'views', 'settings', 'ingest-sources-content.html'].join('/');
    ...
    describe('edit() method', () => {
        ...
        beforeEach((done) => {
            fakeProvider = {
                ...
                feeding_service: 'rss',
                config: {
                    field_aliases: [{foo2: 'bar2'}, {foo4: 'bar4'}],
                },
            };
        ...
        it("updates the list of field names that don't have an alias set", () => {
            scope.edit(fakeProvider);
            expect(scope.fieldsNotSelected).toEqual({field_aliases: ['foo1', 'foo3']});
        });
    ...
    describe('removeFieldAlias() and fieldSelectionChanged() methods', () => {
        ...
        it(
            'does not modify the list of not selected fields if removed ' +
    ...
    describe('save() method', () => {
        ...
        it('updates field aliases in provider configuration', () => {
            ...
            scope.save();
            expect(scope.provider.config.field_aliases).toEqual([{foo1: 'bar1'}, {foo2: 'bar2'}]);
        });
