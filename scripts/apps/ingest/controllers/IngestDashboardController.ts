...
IngestDashboardController.$inject = ['$scope', 'ingestSources', 'preferencesService', 'notify'];
export function IngestDashboardController($scope, ingestSources, preferencesService, notify) {
    ...
    $scope.setUserPreferences = function (refresh) {
        ...
        _.forEach(_.filter($scope.items, {dashboard_enabled: true}), (item) => {
            preferences.push(_.pick(item, _.union(['_id'], _.keys(appConfig.ingest.PROVIDER_DASHBOARD_DEFAULTS))));
        });

        update['dashboard:ingest'] = preferences;
        preferencesService.update(update).then(
            (result) => {
                if (refresh) {
                    $scope.fetchItems();
                }
