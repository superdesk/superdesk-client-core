 ...
 */
export const ContactInfo: React.FunctionComponent<IProps> = ({item, showIfContactIsInactive, hideHeader}) => {
    const contactJobTitle = item.job_title ? <JobTitle item={item} /> : null;
    const contactOrg = item.first_name && item.organisation ? <span>{item.organisation}</span> : null;
                ...
                <Spacer style={{margin: '8px 0'}} h gap="8" justifyContent="start" alignItems="center" noWrap>
                    ...
                    <div key="contact-org">
                        ...
                        {!item.is_active && showIfContactIsInactive && (
                            <span title="inactive" className="label label--draft label--hollow pull-right">
                                {gettext('Inactive')}
                            </span>
                        )}
                    </div>
