...
export class ItemContainer extends React.Component<any, any> {
    ...
    getEmailValue(item) {
        const emails = map(item.contact_email, (email) => (
            <a href={`${MAILTO_URL}${email}`} title={`${email}`}>
                {email}
            </a>
        ));
    ...
    init(props) {
        ...
        switch (key) {
            ...
            case 'twitter':
                value = (
                    <a href={`${TWITTER_URL}${item.twitter}`} target="_blank" rel="noopener noreferrer">
            ...
            case 'location':
                value = contactAddress ? (
                    <a href={`${MAP_URL}${contactAddress}`} target="_blank" rel="noopener noreferrer">
                        {contactAddress}
                    </a>
                ) : null;
    ...
    render() {
        return <span {...this.elemProps}>{this.elemValue}</span>;
    }
}
