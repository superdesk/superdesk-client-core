...
angular
    .module('superdesk.apps.archive.directives', [
        ...
        function (superdesk, workspaceMenuProvider) {
            superdesk
                ...
                .activity('upload.media', {
                    ...
                    filters: [{action: 'upload', type: 'media'}],
                    additionalCondition: [
                        '$location',
                        function ($location) {
                ...
                .activity('spike', {
                    ...
                    additionalCondition: [
                        ...
                        function (session, authoring, item) {
                            return (
                                authoring.itemActions(item).spike &&
                                (item.lock_user === null ||
                                    angular.isUndefined(item.lock_user) ||
                ...
                .activity('duplicateInPlace', {
                    ...
                    additionalCondition: [
                        ...
                        function (authoring, item) {
                            return (
                                authoring.itemActions(item).duplicate &&
                ...
                .activity('duplicateTo', {
                    ...
                    additionalCondition: [
                        ...
                        function (authoring, item) {
                            return (
                                (item.state !== 'killed' || item.state !== 'recalled') &&
                                !authoring.isContentApiItem(item) &&
                                authoring.itemActions(item).duplicateTo
                            );
                        },
                    ],
                    group: 'duplicate',
                ...
                .activity('duplicateToPersonal', {
                    ...
                    additionalCondition: [
                        ...
                        function (authoring, item) {
                            return (
                                authoring.itemActions(item).copy &&
                    ...
                    additionalCondition: [
                        ...
                        function (
                            ...
                            return (
                                (item.state !== 'killed' || item.state !== 'recalled') &&
                                !authoring.isContentApiItem(item) &&
                                authoring.itemActions(item).set_label &&
                                openedItem &&
                                openedItem.type === 'composite' &&
                                packages.isAdded(openedItem, item) &&
                                vocabularies.isInit()
                            );
                        },
                    ],
                ...
                .activity('createBroadcast', {
                    ...
                    controller: [
                        ...
                        function (api, $rootScope, data, desks, authoringWorkspace: AuthoringWorkspaceService) {
                            api.save('archive_broadcast', {}, {desk: desks.getCurrentDeskId()}, data.item).then(
                                (broadcastItem) => {
                ...
                .activity('copy', {
                    ...
                    controller: [
                        ...
                        'data',
                        '$rootScope',
                        (api, data, $rootScope) => actions.copy(data.item, api, $rootScope),
                    ],
                    filters: [{action: 'list', type: 'archive'}],
                    condition: (item: IArticle) => item.lock_user == null && item.task?.desk == null,
                    additionalCondition: [
...
function spikeActivity(
    ...
    const checkIfHasUnsavedChanges =
        data.item.lock_user == null ? Promise.resolve(false) : autosave.hasUnsavedChanges(data.item);
    ...
    function _spike() {
        if (sdApi.navigation.isPersonalSpace()) {
            return modal
                .confirm(gettext('Do you want to delete the item permanently?'), gettext('Confirm'))
                .then(() => sdApi.article.doSpike(data.item));
        }

        const onSpikeMiddlewares: Array<IExtensionActivationResult['contributions']['entities']['article']['onSpike']> =
            flatMap(
