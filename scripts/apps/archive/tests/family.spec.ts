describe('family service', () => {
    ...
    describe('fetching family items', () => {
        ...
        beforeEach(inject((api, $q) => {
            ...
            spyOn(api, 'query').and.callFake((endpoint, params) => {
                ...
                if (params.source.query.filtered.filter.and[2]) {
                    _.remove(members, {unique_id: params.source.query.filtered.filter.and[2].not.term.unique_id});
                }
        ...
        it('can fetch members of a family', (done) =>
            inject(($rootScope, familyService, api) => {
                familyService.fetchItems('family1').then((members) => {
        ...
        it('can fetch members of a family with exclusion', (done) =>
            inject(($rootScope, familyService, api) => {
                familyService.fetchItems('family1', {unique_id: 1, _id: 'z'}).then((members) => {
        ...
        it('can fetch desks of members of a family', (done) =>
            inject(($rootScope, familyService) => {
                familyService.fetchDesks({_id: 'z', family_id: 'family1'}).then((memberDesks) => {
        ...
        it('can determine weather a user is member of fetched desk', (done) =>
            inject(($rootScope, familyService) => {
                familyService
                    .fetchDesks({_id: 'z', family_id: 'family1', task: {desk: 'desk1'}})
                    .then((memberDesks) => {
                        expect(memberDesks.length).toBe(1);
        ...
        it('can fetch desks of members of a family with exclusion', (done) =>
            inject(($rootScope, familyService) => {
                familyService.fetchDesks({unique_id: 1, _id: 'z', family_id: 'family1'}, true).then((memberDesks) => {
        ...
        it('can use item._id for ingest items instead of family id', inject(($rootScope, $q, familyService) => {
    ...
    describe('fetching related items', () => {
        ...
        it('can query related items', (done) =>
            inject(($rootScope, $q, familyService, api) => {
                const query = {
        ...
        it('can query relatable items with empty match criteria', (done) =>
            inject(($rootScope, $q, familyService, api) => {
                const query = {
                    repo: 'archive,published',
                    source: {
        ...
        it('can query relatable items with prefix criteria', (done) =>
            inject(($rootScope, $q, familyService, api) => {
                const query = {
                    repo: 'archive,published',
                    source: {
        ...
        it('can query relatable items with date range', (done) =>
            inject(($rootScope, $q, familyService, api) => {
                const query = {
                    repo: 'archive,published',
                    source: {
