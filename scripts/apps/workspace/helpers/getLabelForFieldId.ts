...
export const getLabelForFieldId = (fieldId, vocabularies) => {
    ...
    const categoriesVocabulary = vocabularies.find((v) => v._id === 'categories');

    if (fieldId === 'anpa_category' && categoriesVocabulary != null) {
        return categoriesVocabulary.display_name;
    }
