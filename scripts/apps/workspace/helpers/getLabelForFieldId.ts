...
export const getLabelForFieldId = (fieldId, vocabularies) => {
    ...
    if (field != null && field.hasOwnProperty('display_name') && field['display_name'].length > 0) {
    ...
    console.warn(
        `could not find label for ${fieldId}. Please create a Custom Vocabulary or add it to:` +
...
export const getLabelNameResolver = () =>
    ng
        .getService('vocabularies')
