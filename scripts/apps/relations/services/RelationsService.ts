...
export const isLink = (association) => association != null && Object.keys(association).length <= RELATED_LINK_KEYS;
...
export function validateWorkflow(
    ...
    if (allowedWorkflows.in_progress !== true && !isPublished(mediaItem)) {
        return {
            result: false,
            error: gettext(
                'Adding related items that are not published ' + 'is not allowed due to configuration options',
            ),
        };
    }
...
export function RelationsService(api, $q) {
    ...
    this.getRelatedItemsForField = function (item: IArticle, fieldId: string) {
        ...
        return $q
            .all(
                relatedItemsKeys.map((key) => {
                    if (isLink(associations[key])) {
                        return api.find('archive', associations[key]._id).catch((response) => {
