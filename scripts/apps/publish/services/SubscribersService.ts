...
export function SubscribersService(api, $q, $filter) {
    ...
    var _getAllSubscribers = function (criteria = {}, page = 1, subscribers = []) {
        return api.query('subscribers', _.extend({max_results: 200, page: page}, criteria)).then((result) => {
            let pg = page;
    ...
    var service = {
        ...
        fetchTargetableSubscribers: function (criteria) {
            return _getAllSubscribers(criteria).then((result) =>
        ...
        initialize: function () {
            if (!this.fetched) {
                this.fetched = _getAllSubscribers().then((subscribers) => {
