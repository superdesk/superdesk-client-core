...
export function HighlightsConfig($scope, highlightsService, desks, api, notify, modal) {
    ...
    $scope.save = function () {
        ...
        highlightsService.saveConfig(_config, $scope.configEdit).then(
            (item) => {
                $scope.message = null;
                if (_new) {
    ...
    $scope.remove = function (config) {
        modal.confirm(gettext('Are you sure you want to delete configuration?')).then(() => {
    ...
    function assignedDesks() {
        return _.map(_.filter($scope.assignedDesks, {included: true}), (desk: any) => desk._id);
    }
