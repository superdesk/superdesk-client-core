...
export const MediaPreview: React.StatelessComponent<IProps> = (props) => {
    ...
    const headlineText = headline.replace(/(<([^>]+)>)/gi, '');
    let preview;

    if (hasThumbnail(props.item)) {
        preview = React.createElement('img', {src: props.item.renditions.thumbnail.href});
    }

    return (
        <div className="media multi">
            {preview ? <figure>{preview}</figure> : null}
            <span className="text">
                <small title={headlineText} dangerouslySetInnerHTML={{__html: headline}} />
                <ItemContainer item={item} desk={props.desk} />
            </span>
