        ...
        link: function ($scope) {
            ...
            searchProviderService.getAllowedProviderTypes().then((providerTypes) => {
                ...
                $scope.addProviderLabel = $scope.noProvidersAllowed
                    ? gettext('There are no providers available.')
                    : gettext('Add New Search Provider');
            ...
            function fetchSearchProviders() {
                searchProviderService.getSearchProviders({manage: 1}).then((result) => {
            ...
            $scope.save = function () {
                ...
                api.search_providers
                    .save($scope.origProvider, $scope.provider)
                    .then(
                        () => {
                        ...
                        (response) => {
                            if (angular.isDefined(response.data._issues)) {
                                if (angular.isDefined(response.data._issues['validator exception'])) {
                                    ...
                                    angular.isDefined(response.data._issues.search_provider) &&
                                    angular.isDefined(response.data._issues.search_provider.unique)
                                ) {
                                    notify.error(
                                        gettext(
                                            'Error: A Search Provider with type ' +
                                                $scope.providerTypes[$scope.provider.search_provider] +
                                                ' already exists.',
                                        ),
                                    );
                                }
            ...
            $scope.remove = function (provider) {
                modal
                    .confirm(gettext('Are you sure you want to delete Search Provider?'))
                    .then(function removeSearchProvider() {
                        api.search_providers
                            .remove(provider)
                            .then(
                                () => {
