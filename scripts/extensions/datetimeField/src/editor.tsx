    ...
    render() {
        ...
        const checkbox = (
            <Switch
                ...
                onChange={(value) => {
                    if (value) {
                        this.props.onChange(dateToServerString(addMinutes(new Date(), config.initial_offset_minutes)));
                    } else {
                        ...
                        {steps.length < 1 && (
                            <Spacer h gap="4" justifyContent="start" noGrow>
                                {steps.map((step, i) => {
                                    ...
                                    if (fullHours > 0) {
                                        buttonText +=
                                            ' ' + gettextPlural(fullHours, '{{x}} hour', '{{x}} hours', {x: fullHours});
                                    }
                                    ...
                                    return (
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (date != null) {
                                                    this.props.onChange(dateToServerString(addMinutes(date, step)));
                                                }
                                            }}
                                            text={buttonText}
                                            style="hollow"
