...
export function updateMarkedUser(superdesk: ISuperdesk, article: IArticle, patch: Partial<IArticle>): void {
    superdesk.entities.article.patch(article, patch, {patchDirectlyAndOverwriteAuthoringValues: true});
}
...
export function markForUserAndSendToNextStage(
    ...
    const getTask = () => {
        ...
        if (task != null && task.desk != null) {
            return superdesk.entities.desk.getStagesOrdered(task.desk).then((stages) => {
