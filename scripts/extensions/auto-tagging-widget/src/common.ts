...
export function getAutoTaggingVocabularyLabels(superdesk: ISuperdesk): Promise<Map<string, string>> /* id, label */ {
    ...
    return dataApi
        .query<IVocabulary>('vocabularies', 1, {field: 'name', direction: 'ascending'}, {imatrics_enabled: true})
        .then((vocabularies) => {
            return Map(vocabularies._items.map(({_id, display_name}) => [_id, display_name]));
        });
}
