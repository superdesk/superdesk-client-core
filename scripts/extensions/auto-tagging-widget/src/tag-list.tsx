    ...
    return class TagList extends React.PureComponent<IProps> {
        render() {
                ...
                return (
                    ...
                    >
                        <Tag
                            ...
                            readOnly={readOnly}
                            shade={
                                savedTags.has(item.qcode) && !readOnly
                                    ? isRootNodeWithChildren
                                        ? 'highlight2'
                                        : 'highlight1'
                                    : isRootNodeWithChildren
                                      ? 'darker'
                            ...
                            onClick={
                                readOnly
                                    ? noop
                                    : () => {
                                          onRemove(treeToArray([node]).map(({qcode}) => qcode));
                                      }
                            }
                        />
                    </TagPopover>
            ...
            function renderTreeNode(treeNodes: Array<ITreeNode<ITagUi>>, level: number = 0): JSX.Element {
                ...
                return !inline ? (
                    <div style={{paddingInlineStart: level === 0 || inline ? 0 : 14}}>{treeNodesMap}</div>
                ) : (
                    <React.Fragment>
                        {level === 0 ? '' : ' | '}
                        {treeNodesMap}
