...
export function setsReducer(state: ISetState = initialState, action: ISetActionTypes): ISetState {
    switch (action.type) {
        case RECEIVE_SETS:
            return {
                ...state,
...
function removeSetFromStore(prevState: ISetState, setToDelete: ISetItem) {
    return {
        ...prevState,
        sets: prevState.sets.filter((set: ISetItem) => set._id !== setToDelete._id),
    };
}
