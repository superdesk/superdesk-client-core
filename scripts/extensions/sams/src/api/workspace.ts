...
export function getDesksSamsSettings(): Promise<Dictionary<IDesk['_id'], IDesk['sams_settings']>> {
    return superdeskApi.dataApi
        .query<IDesk>('desks', 1, {field: '_id', direction: 'ascending'}, {})
        .then((response) => {
            return response._items.reduce<Dictionary<IDesk['_id'], IDesk['sams_settings']>>((items, desk) => {
...
export function getSingleDeskSamsSettings(deskId: IDesk['_id']): Promise<IDesk['sams_settings']> {
    return superdeskApi.dataApi.findOne<IDesk>('desks', deskId).then((desk) => {
...
export function updateSetsAllowedDesks(setId: ISetItem['_id'], allowedDesks: Array<IDesk['_id']>): Promise<void> {
    ...
    const toRemove = currentlyAllowed.filter((deskId) => !allowedDesks.includes(deskId));
    const toAdd = allowedDesks.filter((deskId) => !currentlyAllowed.includes(deskId));
            ...
            return Promise.all(
                response._items.map((original) => {
                    ...
                    if (toAdd.includes(original._id)) {
                        ...
                        samsSettings.allowed_sets = samsSettings.allowed_sets.filter((sId) => sId !== setId);
                    }

                    return superdeskApi.dataApi.patch<IDesk>('desks', original, {sams_settings: samsSettings});
                }),
