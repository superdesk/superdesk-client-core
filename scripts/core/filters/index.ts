...
export default angular
    ...
    .filter('formatLocalDateTimeString', [
        ...
        function (moment) {
            return function (input, formatString) {
                ...
                return angular.isDefined(formatString)
                    ? momentTimestamp.local().format(formatString)
    ...
    .filter('parseDateline', [
        'moment',
        (moment) =>
            function (dateToFormat, located) {
    ...
    .filter('relativeUTCTimestamp', [
        'moment',
        (moment) =>
            function (located, month, date) {
    ...
    .filter(
        'sortByName',
        () =>
            function (_collection, propertyName = 'name') {
                return _.sortBy(_collection, (_entry) =>
                    _entry[propertyName] ? _entry[propertyName].toLowerCase() : _entry.name.toLowerCase(),
                );
            },
    )
    .filter(
        ...
        () =>
            function (filterCondition, valueLookup) {
                var labels = [];
