...
export class SuperdeskReactComponent<IProps = {}, IState = {}> extends React.PureComponent<IProps, IState> {
    ...
    constructor(props: IProps) {
        ...
        this.asyncHelpers = {
            fetch: (input: RequestInfo, init?: RequestInit): Promise<Response> => {
                return ignoreAbortError(fetch(input, {...(init ?? {}), signal: abortController.signal}));
            },
            httpRequestJsonLocal: (options: Parameters<typeof httpRequestJsonLocal>[0]) => {
                return ignoreAbortError(httpRequestJsonLocal({...options, abortSignal: abortController.signal}));
            },
        };
