...
export default angular
    .module('superdesk.core.upload.imagecrop', ['superdesk.core.translate'])
                ...
                link: function (scope, elem) {
                    ...
                    function parseCoordinates(cropImage) {
                        if (!_.isNil(cropImage) && !_.isNil(cropImage.CropLeft)) {
                            return [cropImage.CropLeft, cropImage.CropTop, cropImage.CropRight, cropImage.CropBottom];
                        }
                    }

                    scope.$watch('src', (src) => {
                        var cropSelect =
                            parseCoordinates(scope.cropInit) ||
                    ...
                    function refreshImage(src, setSelect) {
                        ...
                        img.onload = function () {
                            if (
                                !src ||
                                (scope.showMinSizeError && !validateConstraints(scope.original, scope.rendition))
                            ) {
                                return;
                            }
                            ...
                            if (angular.isDefined(scope.rendition)) {
                                if (angular.isDefined(scope.rendition.ratio)) {
                                    ...
                                    angular.isDefined(scope.rendition.width) &&
                    ...
                    function validateConstraints(_img, rendition) {
                        ...
                        if (_img.width < rendition.width || _img.height < rendition.height) {
                            var text =
                                gettext('Sorry, but image must be at least {{width}}x{{height}}.', {
                                    width: rendition.width,
                                    height: rendition.height,
                                }) +
                                ' ' +
                                gettext('Currently the image size is {{width}}x{{height}}).', {
                                    width: _img.width,
                                    height: _img.height,
                                });
