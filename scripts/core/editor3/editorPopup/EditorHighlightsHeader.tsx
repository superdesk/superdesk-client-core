...
export class EditorHighlightsHeader extends React.Component<IProps, IState> {
    ...
    render() {
        ...
        const actionsDropdownStyles: any =
            this.state.actionsDropdownOpen !== true
                ? {}
                : {
                      display: 'block',
                      position: 'absolute',
        ...
        return (
            <div className="editor-popup__header">
                <div className="editor-popup__header-text">{this.props.children}</div>

                {availableActions.length < 1 ? null : (
                    <div className="editor-popup__header-tools">
                        <div className="dropdown dropdown--align-right">
                            <button className="icn-btn dropdown__toggle" onClick={() => this.toggleActionsDropdown()}>
