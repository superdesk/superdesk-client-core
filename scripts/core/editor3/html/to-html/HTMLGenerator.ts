...
export class HTMLGenerator {
    ...
    constructor(contentState, disabled = []) {
        ...
        // A set of disabled elements (ie. ['table'] will ignore
        this.disabled = disabled || [];
    }

    html() {
        const annotationsByStyleName = getAnnotationsFromContentState(this.contentState)
        ...
        let options = {
            ...
            blockRenderers: {
                atomic: (block: ContentBlock) => new AtomicBlockParser(this.contentState, this.disabled).parse(block),
            },
        ...
        */
        var res = trimStartExact(
            trimEndExact(
                stateToHTML(this.contentState, options),
                '<p><br></p>',
            ),
            '<p><br></p>',
        ).trim();
