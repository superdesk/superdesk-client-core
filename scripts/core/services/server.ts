...
export default angular
    .module('superdesk.core.services.server', [])
    /**
     * @ngdoc service
        ...
        function ($q, $http) {
            return {
                ...
                create: function (resource, data) {
                    return this._http('post', this._makeUrl(resource), null, data);
                },
                ...
                list: function (resource, params) {
                    return this._http('get', this._makeUrl(resource), this._convertParams(params));
                },

                // transfer url params to server params
                _convertParams: function (params) {
                    var serverParams: any = _.extend(
                        {},
                        _.pick(params, ['filter', 'max_results', 'page', 'embedded', 'where', 'q', 'df']),
                    );

                    if ('sort' in params) {
                        serverParams.sort =
                            '[(' + angular.toJson(params.sort[0]) + ',' + (params.sort[1] === 'asc' ? 1 : -1) + ')]';
                    }
                 ...
                 */
                read: function (item) {
                    return this._http('get', this._wrapUrl(item._links.self.href));
                },
                ...
                readById: function (resource, id) {
                    return this._http('get', this._makeUrl(resource, id));
                },
                ...
                update: function (item, data) {
                    ...
                    return this._http('patch', this._wrapUrl(item._links.self.href), {}, item);
                },
                /**
                 * @ngdoc method
                 * @name server#delete
                ...
                delete: function (item) {
                    return this._http('delete', this._wrapUrl(item._links.self.href), {}, item);
                },
            };
        },
    ]);
