...
function prepareAndPrint() {
    ...
    const afterPrintFns: Array<() => void> = [() => document.body.classList.remove('prepare-to-print')];
 ...
 * Usage example:
    ...
    />
 */
export function showPrintableModal(Component: React.ComponentType<IPropsPrintableModal>) {
    showModal(({closeModal}) => {
        return (
            <Component
                Wrapper={({toolbar, contentSections}) => (
                    <div className="sd-full-preview">
                        <div className="sd-full-preview--header no-print">{toolbar}</div>
                            ...
                            {contentSections.map((section, i) => {
                                return (
                                    <div key={i}>
                                        ...
                                        <div className="sd-full-preview--content">{section}</div>
                                    </div>
                                );
                            })}
                        </div>
...
function getErrorsModal(title: string, errors: Array<string>, description?: string) {
        ...
        render() {
            return (
                <Modal
                    ...
                    footerTemplate={<Button type="primary" text={gettext('Ok')} onClick={this.props.closeModal} />}
                >
                    {description == null ? null : <h3>{description}</h3>}
...
export function showErrorsModal(title: string, errors: Array<string>) {
    showModal(getErrorsModal(title, errors));
}

export default angular
    .module('superdesk.core.services.modal', [
        'superdesk-ui',
    ...
    .service('modal', [
        ...
        function ($q, $modal, $sce, asset) {
            ...
            function confirmBase(bodyText, headerText, okText, cancelText, additionalCancelText) {
                var delay = $q.defer();
            ...
            this.prompt = function (title, initialValue) {
                return new Promise((resolve, reject) => {
                    this.createCustomModal().then(({openModal, closeModal}) => {
                        openModal(
                            <ModalPrompt
                                title={title}
