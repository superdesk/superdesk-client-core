...
export function getNativeKey(superdeskKey) {
    return (
        Object.keys(superdeskKeyboardKeyNamesConvention).find(
            (key) => superdeskKeyboardKeyNamesConvention[key] === superdeskKey,
        ) ?? superdeskKey
    );
}
...
export default angular
    .module('superdesk.core.keyboard', [])
    ...
    .run([
        ...
        function KeyEventBroadcast($rootScope, $document, Keys, shiftNums) {
            ...
            $document.on('keydown', (e) => {
                ...
                if (isGlobal || !ignoreNodes[e.target.nodeName]) {
                    // $document.body is empty when testing
    ...
    .service('keyboardManager', [
        '$window',
        '$timeout',
        'shiftNums',
        function ($window, $timeout, shiftNums) {
            var stack = [],
                defaultOpt = {
                    type: 'keydown',
                    propagate: true,
