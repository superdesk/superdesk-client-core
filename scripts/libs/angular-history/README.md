...
_Fair warning:_ Until this project is at `1.0.0` the API is subject to change
...
`History` service will broadcast. You can use these _chainable_ objects to run
...
Once something is `watch()`ed, you can undo or redo changes to it, _if that
expression is assignable_. If you pass a function as the expression, you may
_not_ undo/redo, but you will still have access to the history stack.
...
History.deepWatch(
    'value.name for (key, value) in foos',
    $scope,
...
History.deepWatch('od.name for (key, od) in otherdata', scope, 'name in otherdata changed');
...
_You do not have to use the `child` variable unless you want to_. It's
