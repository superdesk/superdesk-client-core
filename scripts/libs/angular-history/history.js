...
(function () {
    ...
    var DEEPWATCH_EXP =
            /^\s*(.*?)\s+for\s+(?:([\$\w][\$\w\d]*)|(?:\(\s*([\$\w][\$\w\d]*)\s*,\s*([\$\w][\$\w\d]*)\s*\)))\s+in\s+(.*?)$/,
    ...
    try {
        ...
        angular.module('lazyBind', []).factory('$lazyBind', function () {
            return angular.noop;
        });
    }
    ...
    angular.module('decipher.history', ['lazyBind']).service('History', [
        ...
        function ($parse, $rootScope, $interpolate, $lazyBind, $timeout, $log, $injector) {
             ...
             */
            Watch.prototype._addHandler = function _addHandler(where, name, fn, resolve) {
                if (!where || !name || !fn) {
             ...
             * @returns {Watch} This {@link decipher.history.object:Watch Watch instance}
             */
            Watch.prototype.addChangeHandler = function addChangeHandler(name, fn, resolve) {
                if (!name || !fn) {
             ...
             * @returns {Watch} This {@link decipher.history.object:Watch Watch instance}
             */
            Watch.prototype.addUndoHandler = function addUndoHandler(name, fn, resolve) {
                if (!name || !fn) {
                    throw new Error('invalid parameters');
                }
             ...
             * @returns {Watch} This {@link decipher.history.object:Watch Watch instance}
             */
            Watch.prototype.addRedoHandler = function addRedoHandler(name, fn, resolve) {
                if (!name || !fn) {
                    throw new Error('invalid parameters');
                }
             ...
             * @returns {Watch} This {@link decipher.history.object:Watch Watch instance}
             */
            Watch.prototype.addRevertHandler = function addRevertHandler(name, fn, resolve) {
                if (!name || !fn) {
                    throw new Error('invalid parameters');
                }
             ...
             * @returns {Watch} This {@link decipher.history.object:Watch Watch instance}
             */
            Watch.prototype.addRollbackHandler = function addRollbackHandler(name, fn, resolve) {
                if (!name || !fn) {
                    throw new Error('invalid parameters');
                }
             ...
             */
            Watch.prototype.removeRollbackHandler = function removeRollbackHandler(name) {
             ...
             */
            Watch.prototype.removeRedoHandler = function removeRedoHandler(name) {
                if (!name) {
                    throw new Error('invalid parameters');
                }
             ...
             * @private
             */
            Watch.prototype._fireHandlers = function _fireHandlers(where, exp, scope) {
             ...
             * @private
             */
            Watch.prototype._fireChangeHandlers = function _fireChangeHandlers(exp, scope) {
             ...
             * @private
             */
            Watch.prototype._fireUndoHandlers = function _fireUndoHandlers(exp, scope) {
             ...
             * @private
             */
            Watch.prototype._fireRedoHandlers = function _fireRedoHandlers(exp, scope) {
             ...
             * @private
             */
            Watch.prototype._fireRevertHandlers = function _fireRevertHandlers(exp, scope) {
             ...
             */
            Watch.prototype.ignoreEvent = function ignoreEvent(eventName, callback, resolve) {
            ...
            Watch.prototype.canUndo = function canUndo() {
                return this.exp === null ? false : service.canUndo(this.exp, this.scope);
            };
            ...
            Watch.prototype.canRedo = function canRedo() {
                return this.exp === null ? false : service.canRedo(this.exp, this.scope);
            };
            ...
            this.watch = function watch(exps, scope, description, lazyOptions) {
                ...
                while (i--) {
                    ...
                    if (isUndefined(model.assign)) {
                        throw 'expression "' + exp + '" is not an assignable expression';
                    }
       ...
       */
            this.deepWatch = function deepWatch(exp, scope, description, lazyOptions) {
                var match,
                    targetName,
                    valueFn,
                    keyName,
                    ...
                    createDeepWatch = function createDeepWatch(targetName, valueName, keyName, watchObj) {
                        return function (values) {
                            forEach(values, function (v, k) {
                                ...
                                if (lazyBindFound && isObject(lazyOptions)) {
                                    watches[id][targetName] = locals.$watch(
                                        lazyWatch(locals, targetName, lazyOptions.timeout || 500),
                ...
                if (!(match = exp.match(DEEPWATCH_EXP))) {
                    throw (
                        'expected expression in form of "_select_ for (_key_,)? _value_ in _collection_" but got "' +
                        exp +
                        '"'
                    );
                }
                ...
                watches[id][targetName] = scope.$watchCollection(
                    valuesName,
                    createDeepWatch(targetName, valueName, keyName, watchObj),
                );
            ...
            this._clear = function _clear(scope, exps) {
                var id = scope.$id,
                    ...
                    clear = function clear(id, key) {
                        ...
                        if (isDefined(watches[id]) && isFunction(watches[id][key])) {
                            watches[id][key]();
                        }
            ...
            this.undo = function undo(exp, scope) {
                ...
                if (isUndefined(stack)) {
                    throw 'could not find history in scope "' + id + ' against expression "' + exp + '"';
                }
                ...
                if (isDefined(watchObjs[id]) && isDefined(watchObjs[id][exp])) {
            ...
            this._watch = function _watch(exp, scope, pass, lazyOptions) {
                ...
                if (lazyBindFound && (isObject(lazyOptions) || (lazyWatches[id] && !!lazyWatches[id][exp]))) {
                    watches[id][exp] = scope.$watch(
                        lazyWatch(scope, exp, lazyOptions.timeout),
                        bind(this, this._archive(exp, id, scope, pass)),
                        true,
                    );
                    lazyWatches[id][exp] = true;
                } else {
                    watches[id][exp] = scope.$watch(exp, bind(this, this._archive(exp, id, scope, pass)), true);
       ...
       */
            this.canUndo = function canUndo(exp, scope) {
                ...
                scope = scope || $rootScope;
                id = scope.$id;
                return isDefined(pointers[id]) && isDefined(pointers[id][exp]) && pointers[id][exp] > 0;
            };
       ...
       */
            this.batch = function batch(fn, scope, description) {
                ...
                listener = scope.$on('History.archived', function (evt, data) {
                    ...
                    if (data.locals.$id !== child.$id) {
                        ...
                        id = deepChild.$id;
                        _initStores(id);
                        history[id][exp] = copy(history[data.locals.$id][exp]);
            ...
            this.rollback = function rollback(t) {
                ...
                function _rollback(scope, comparisonScope) {
                    ...
                    while (i--) {
                        ...
                        while (pointer > pointers[id][exp]) {
                            pointer--;
                            values.push(_do(comparisonScope, exp, history[comparisonScopeId][exp], pointer));
                ...
                if (isDefined(parent) && isDefined(history[parent.$id])) {
                    _rollback(t, parent);
                }
