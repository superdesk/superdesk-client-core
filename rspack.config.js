...
module.exports = function makeConfig(grunt) {
    ...
    if (grunt && grunt.option && grunt.option('config')) {
                    ...
                    use: [
                        {
                            ...
                            options: {
                                ...
                                sassOptions: {
                                    importers: [
                                        {
                                            // Handle ~ prefixed imports and bare imports
                                            findFileUrl(url) {
                                                // Remove ~ prefix if present
                                                const cleanUrl = url.replace(/^~/, '');

                                                // Try to resolve from includePaths
                                                const includePaths = [
                                                    path.join(__dirname, 'styles', 'sass'),
                                                    path.join(__dirname, 'scripts'),
                                                    __dirname,
                                                ...
                                                for (const includePath of includePaths) {
                                                    const possiblePaths = [
                                                        path.join(includePath, cleanUrl),
                                                        path.join(includePath, `_${cleanUrl}`),
                                                        path.join(includePath, cleanUrl.replace(/\.scss$/, ''), '_index.scss'),
                                                        path.join(includePath, cleanUrl.replace(/\.scss$/, ''), 'index.scss'),
                                                    ];
...
function getDefaults(grunt) {
    ...
    return {
        version: version || (grunt && grunt.file ? grunt.file.readJSON(path.join(__dirname, 'package.json')).version : require('./package.json').version),

        server: {
            url: (grunt && grunt.option ? grunt.option('server') : null) || process.env.SUPERDESK_URL || 'http://localhost:5000/api',
            ws: (grunt && grunt.option ? grunt.option('ws') : null) || process.env.SUPERDESK_WS_URL || 'ws://0.0.0.0:5100',
        },
        ...
        editor: {
            ...
            disableEditorToolbar: (grunt && grunt.option ? grunt.option('disableEditorToolbar') : null),
        },
        ...
        isTestEnvironment: !!(grunt && grunt.option && grunt.option('environmentName')) || !!process.env.SUPERDESK_ENVIRONMENT,

        // environment name
        environmentName: (grunt && grunt.option ? grunt.option('environmentName') : null) || process.env.SUPERDESK_ENVIRONMENT,
