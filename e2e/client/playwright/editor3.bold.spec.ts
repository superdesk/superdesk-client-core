...
test.describe('bold formatting in the article body', () => {
    const HEADLINE = 'bold formatting test';
    ...
    const BOLD_HEAD = 'alpha';
    const BOLD_TAIL = 'bravo';
    const REGULAR_HEAD = 'charlie';
    const REGULAR_TAIL = 'delta';

    const BOLD_TEXT = BOLD_HEAD + BOLD_TAIL;
     ...
     * the weight computed on the text node reflects them through inheritance.
     */
    function textRun(body: Locator, text: string): Locator {
        return body.locator('[data-text="true"]').filter({hasText: text});
    }

    // Asserted as "not bold" rather than a literal weight: the editor's base font
    // weight is a theme value, while `bold` always computes to 700.
    async function expectBold(run: Locator, bold: boolean): Promise<void> {
        if (bold) {
            await expect(run).toHaveCSS('font-weight', BOLD_WEIGHT);
        } else {
            await expect(run).not.toHaveCSS('font-weight', BOLD_WEIGHT);
        }
    }
    ...
    test('toolbar toggle bolds typed text and a selection, and the result survives reopen', {
        annotation: [
            ...
            {type: 'confluence', description: '1313669361 partial'}, // Bold
        ],
    }, async ({page}) => {
        ...
        const boldButton = body.getByTestId('toolbar')
            .getByTestId('formatting-option-button')
            .and(page.locator('[data-test-value="bold"]'));

        await expect(bodyInput).toBeVisible();
        await bodyInput.click();

        await expect(boldButton).not.toHaveClass(ACTIVE_BUTTON);
        await boldButton.click();
        await expect(boldButton).toHaveClass(ACTIVE_BUTTON);

        await page.keyboard.type(BOLD_TEXT);
        await expectBold(textRun(body, BOLD_TEXT), true);

        await boldButton.click();
        await expect(boldButton).not.toHaveClass(ACTIVE_BUTTON);

        await page.keyboard.type(REGULAR_TEXT);
        await expectBold(textRun(body, REGULAR_TEXT), false);
        await expectBold(textRun(body, BOLD_TEXT), true);
        ...
        await boldButton.click();
        await expect(boldButton).toHaveClass(ACTIVE_BUTTON);
        await expectBold(textRun(body, REGULAR_TAIL), true);
        await expectBold(textRun(body, REGULAR_HEAD), false);

        // Collapse the selection to its start, walk back over the regular run and
        // select the second half of the bold run. Counted arrow presses rather than
        // Home/End, which do not move the caret on macOS.
        await page.keyboard.press('ArrowLeft');
        await pressRepeatedly(page, 'ArrowLeft', REGULAR_HEAD.length);
        await pressRepeatedly(page, 'Shift+ArrowLeft', BOLD_TAIL.length);

        await expect(boldButton).toHaveClass(ACTIVE_BUTTON);
        await boldButton.click();
        await expect(boldButton).not.toHaveClass(ACTIVE_BUTTON);
        await expectBold(textRun(body, BOLD_TAIL), false);
        await expectBold(textRun(body, BOLD_HEAD), true);
